...
module hr
    ...
    @everywhere function ftransform(hr,kpoints::Array{T}) where{T}
...
module hamkmesh
    ...
    mutable struct Hamkmesh{T}
        ...
        function Hamkmesh(kmesh,hr::HR{T}) where T
            ...
            hkm.hr = hr
            #=
            hkm.hk_list = [zeros(Complex{T},(hkm.hr.num_wann,hkm.hr.num_wann))
                           for i in 1:hkm.nk]
            generate_hk!(hkm)=#
            hkm
        end
    ...
    #=
    function apply_func_ksum!(result::Vector,hkm::Hamkmesh,func)
    ...
    function apply_func_ksum!(result,hkm::Hamkmesh,func)
        result = pmap(func,hkm.kp_list)
    end
end
...
module cpa
    ...
    @everywhere function calc_green_func_k(cpa,k::Array)
    ...
    @everywhere function calc_tr_gkiwn(cpa,k::Array)
        tr.(calc_green_func_k(cpa,k))
    end

    function calc_ne(cpa,mu)
        println("aaa")
        @everywhere cpa.mu = mu
        trg = zeros(Complex,size(cpa.temp.omegaF_sp)[1])
        println("fxxk2")
        @everywhere applyfunc(k) = calc_tr_gkiwn(cpa,k)/(cpa.cpa_hkm.nk)
        println("trg")
        apply_func_ksum!(trg,cpa.cpa_hkm,applyfunc)
        println("gl")
    ...
    function calc_green_function(cpa::CPA)
        ...
        println("Fxxk")
        @everywhere g = (x -> calc_green_func_k(cpa,x))
        println("fxxk!")
        cpa.mu = find_zero(f,(-dmu+cpa.mu,cpa.mu+dmu),Bisection())
        println("green cal")
        result = [zeros(cpa.num_wann,cpa.num_wann) for i in 1:size(cpa.temp.omegaF_sp)[1]]
        @time cpa.green[:] = apply_func_ksum!(result,cpa.cpa_hkm,g)/cpa.cpa_hkm.nk
    end

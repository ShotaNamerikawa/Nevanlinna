...
function make_hr_from_file(hrfile::String;T=Float64)
    fp = open(hrfile)
    weight = Int[]
    readline(fp)
...
function ftransform(hr::Hr{T},kpoints::Array) where{T} 
    ...
    for j in 1:size(hr.hopping,2)
        for i in 1:size(hr.hopping,1)
            hkw[i,j] = transpose(phasefactor)*(hr.hopping[i,j,:] ./ hr.weight)
        end
    end
    hkw
end
...
function generate_hkwa(hr::Hr{T},kpoints::Array) where {T}
    phase = hr.rpoints * kpoints
    phasefactor=exp.((2.0im*pi).*phase) ./ hr.weight
    hkwa = zeros(Complex{T},(size(hr.hopping,1),size(hr.hopping,2),3))
    for l in 1:3
        weight = 1.0im*hr.rpoints[:,l].*phasefactor
        for j in 1:size(hr.hopping,2)
            for i in 1:size(hr.hopping,1)
                hkwa[i,j,l] = transpose(weight)*hr.hopping[i,j,:]
            end
        end
    end
    hkwa
...
function generate_hkha(hkwa::Array{Complex{T},3},U::Array{Complex{T},2}) where {T}
    hkha = zeros(Complex{T},size(hkwa))
    @tensoropt hkha[i,j,k] = conj(U)[l,i]*hkwa[l,m,k]*U[m,j] 
end

function generate_va(hkwa::Array{Complex{T},3},U::Array{Complex{T},2}) where {T}
    hkha = zeros(Complex{T},size(hkwa,1),3)
    for i in 1:size(hkwa,1)
        @tensoropt hkha[i,:][k] = conj(U)[:,i][l]*hkwa[l,m,k]*U[:,i][m]
    end
    hkha 
end
